...
export function CreditCardVisual({ account, compact = false }: CreditCardVisualProps) {
  ...
  if (compact) {
    return (
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${account.colorCode}dd, ${account.colorCode}88)`,
          minWidth: 200,
        }}
      >
        <div className="flex justify-between items-start mb-2">
          <span className="text-xs font-medium opacity-80">{account.institutionName || account.name}</span>
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(135deg, ${account.colorCode}ff 0%, ${account.colorCode}99 60%, ${account.colorCode}44 100%)`,
        minHeight: 200,
        maxWidth: 400,
      }}
    >
      ...
      <div className="flex justify-between items-start relative z-10">
        <div>
          <p className="text-xs font-medium opacity-70 uppercase tracking-wider">
            {account.institutionName || 'Personal'}
          </p>
      ...
      <div className="mt-4 flex justify-between items-end relative z-10">
        <div>
          <p className="text-xs opacity-60 uppercase tracking-wide">
            {account.cardHolderName || 'Titular'}
          </p>
          {account.expiryDate && (
